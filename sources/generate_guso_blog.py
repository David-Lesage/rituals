...
.art a:not(.btn){color:var(--plum2);text-decoration:underline;text-decoration-color:rgba(179,143,245,.5);text-underline-offset:3px}
...
.art blockquote{position:relative;overflow:hidden;margin-top:26px;padding:22px 24px;border:1px solid rgba(179,143,245,.26);border-radius:18px;background:linear-gradient(135deg,rgba(147,116,226,.12),rgba(238,128,98,.06))}
...
.final{position:relative;overflow:hidden;margin-top:52px;padding:38px 34px 34px;border:1px solid rgba(255,255,255,.09);border-radius:26px;background:linear-gradient(135deg,rgba(216,178,90,.12),rgba(238,128,98,.10) 48%,rgba(147,116,226,.12));box-shadow:0 30px 70px -46px rgba(0,0,0,.95)}
...
.carte:hover{transform:translateY(-3px);border-color:rgba(248,210,116,.34)}
...
.theme-ico{flex:0 0 auto;width:42px;height:42px;display:flex;align-items:center;justify-content:center;border-radius:14px;border:1px solid rgba(248,210,116,.22);background:linear-gradient(140deg,rgba(216,178,90,.16),rgba(238,128,98,.12) 55%,rgba(147,116,226,.14))}
...
def _controle_page(html, nom):
    ...
    #
    # ⚠️ 16/08/2026 — CE CONTROLE FIGEAIT DEUX VALEURS EN DUR (`--coral:#e08a72`
    #    et `--plum2:#b3a2e4`). La refonte de la palette a change les deux, et
    #    les 19 pages du blog ont ete REFUSEES A L'ECRITURE : le garde-fou
    #    protegeait la couleur au lieu de proteger la couche. C'est exactement
    #    le piege deja documente pour `NAV_VERSION` dans `verif_commentaires.py`
    #    — « ne jamais y figer un numero ». On lit donc la valeur ATTENDUE dans
    #    `theme_chaleur.CSS`, sa seule source de verite. Le controle garde tout
    #    son mordant (une couche absente ou incomplete est toujours refusee)
    #    sans se perimer a chaque retouche de palette.
    attendus = [('--grad:linear-gradient', 'le degrade signature --grad'),
                ('body::before', 'les trois halos de fond')]
    for variable in ('--coral', '--plum2'):
        trouve = re.search(re.escape(variable) + r':(#[0-9a-fA-F]{3,6})',
                           theme_chaleur.CSS)
        if not trouve:
            ennuis.append('theme_chaleur.CSS ne declare plus %s' % variable)
            continue
        attendus.append(('%s:%s' % (variable, trouve.group(1)),
                         'la couleur %s' % variable))
    for reperage, quoi in attendus:
