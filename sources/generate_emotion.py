...
""")
# --------------------------------------------------------------------------
# LA VISIONNEUSE PHOTO (17/08/2026) — voir aussi VISIONNEUSE_JS plus bas
# --------------------------------------------------------------------------
...
#
# ⚠️ AUCUNE de ces regles n'est chargee de « faire tenir » la page : sans
#    JavaScript rien ne s'ouvre, et la page reste exactement ce qu'elle etait.
#    C'est pour cela que le dialogue N'EXISTE PAS dans le HTML livre — il est
#    construit par le script. Le HTML publie ne gagne donc pas un caractere de
#    texte, seulement un bloc <style> et un bloc <script>.
#
# ⚠️ `.gal-item .c{pointer-events:none}` N'EST PAS COSMETIQUE. La legende des
#    photos de la galerie est en `position:absolute` PAR-DESSUS le bas de
#    l'image : sans cette ligne, tout le bas de chaque photo n'est pas
#    cliquable. Mesure faite avant de l'ecrire.
#
...
/* visionneuse photo */
"""
 # z-index 1100 : au-dessus du panneau du menu mobile (1001) et de son
...
"""
 # le defilement des photos, c'est le defilement NATIF du navigateur, avec
...
"""
 # Cible tactile des quatre boutons : 46 px et non 44. Mesure du 17/08/2026 :
...
"""
 # la legende des photos de la galerie couvre le bas de l'image : sans ceci
 # tout ce bas n'est pas cliquable
 """.gal-item .c{pointer-events:none}
.ph-op{cursor:zoom-in}
...
#
# Voir la note detaillee au-dessus du bloc CSS `.ph`. Ce qu'il faut savoir ici :
#
...
#   le test `canvas.toDataURL` de `generate_site.py` — meme technique sur les
#   deux pages.
...
r"""<script>
(function(){
  var SEL='.hero .banner img, .affiche img, .gal-item img, .figure img, picture.aphoto img';
