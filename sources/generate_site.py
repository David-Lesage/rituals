...
with open('site_public/index.html','r',encoding='utf-8') as f:
    html=f.read()
...
html=html.replace(anchor_voyage, figblock(fig_exp,'Au Grand Rex devant 2700 personnes')+anchor_voyage,1)

# journey figure + keystone bg
html=html.replace('<section class="keystone"><div class="wrap">',
  figblock(fig_journey,'L’induction — la voix qui guide, vers un état de conscience élargie')+
...
slides=''
for tok,cap in GAL:
    slides+='      <div class="slide"><img onclick="openIMG(this)" src="'+promo(tok,1500,80)+'" alt=""><span class="cap2">'+cap+'</span></div>\n'
...
with open('RITUALS_site.html','w',encoding='utf-8') as f:
    f.write(html)
print('WROTE RITUALS_site.html', round(len(html)/1024),'KB')
