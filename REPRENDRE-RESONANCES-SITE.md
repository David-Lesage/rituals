...
- **⏸️ MIS EN PAUSE par David (03/08 soir)** — Agent calendrier /le-nid arrêté PENDANT sa phase de vérification navigateur : les modifications sont écrites mais NON committées ni vérifiées (`git status` : `le-nid/index.html` et `sources/generate_agenda_nid.py` modifiés, en l'état dans le working tree). À la reprise : relire le diff, finir la vérification (serveur local + Chrome, mobile 390×844, grep le code portail = 0), puis committer et montrer à David. Périmètre : boutons « Ajouter à mon Google Agenda » par événement (en PLUS des .ics conservés ; jamais le code portail, grep doit rester 0) + filtres par type et par mois + ancre #concerts pré-filtrée = vue « toutes les dates de concerts ». Commit local sans push attendu. → À la reprise : lire son résultat, VÉRIFIER le rendu, montrer à David avant push. Limitation actée avec David : les liens Google TEMPLATE ne peuvent pas imposer de rappels (rappels = .ics + abonnement calendrier).
